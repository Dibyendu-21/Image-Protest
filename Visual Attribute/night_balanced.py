# -*- coding: utf-8 -*-
"""
Created on Sun Apr 28 20:15:10 2019
...
def read(filename):
    df = pd.read_csv(filename, nrows=2762)
    print(df.head())
    label = df['night']
...
train_data,train_target=read('night_balanced.csv')
...
def gen2():
    ...
    while True: 
        for i in range(0,79):
            iteru = []
            iteru1 = []
            itera = islice(data_test, i*7, (i+1)*7)
            itera1 = islice(label_test, i*7, (i+1)*7)
            for img in itera:
                iteru.append(img)
            for img in itera1:
                iteru1.append(img)
...
tr_gen1 = gen1()
tr_gen2 = gen2()
m_check=keras.callbacks.ModelCheckpoint(filepath='./night_augment500.h5',monitor='val_fmeasure', verbose=0, save_best_only=True, mode='min')
