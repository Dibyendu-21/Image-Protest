# -*- coding: utf-8 -*-
"""
Created on Sun Apr 28 19:44:15 2019
...
def read(filename):
    df = pd.read_csv(filename, nrows=4464)
    print(df.head())
    label = df['flag']
...
train_data,train_target=read('flag_balanced.csv')
...
def gen():
    ...
    while True: 
        for i in range(0,119):
            iteru = []
            iteru1 = []
            itera = islice(data_train, i*24, (i+1)*24)
            itera1 = islice(label_train, i*24, (i+1)*24)
            for img in itera:
                iteru.append(img)
            for img in itera1:
                iteru1.append(img)
...
def gen1():
    ...
    while True: 
        for i in range(0,34):
            iteru = []
            iteru1 = []
            itera = islice(data_holdout, i*21, (i+1)*21)
            itera1 = islice(label_holdout, i*21, (i+1)*21)
            for img in itera:
                iteru.append(img)
            for img in itera1:
                iteru1.append(img)
...
def gen2():
    ...
    while True: 
        for i in range(0,47):
            iteru = []
            iteru1 = []
            itera = islice(data_test, i*19, (i+1)*19)
            itera1 = islice(label_test, i*19, (i+1)*19)
            for img in itera:
                iteru.append(img)
            for img in itera1:
                iteru1.append(img)
...
tr_gen1 = gen1()
tr_gen2 = gen2()
m_check=keras.callbacks.ModelCheckpoint(filepath='./flag_augment500.h5',monitor='val_fmeasure', verbose=0, save_best_only=True, mode='min')
