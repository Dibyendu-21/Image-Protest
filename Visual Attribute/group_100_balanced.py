# -*- coding: utf-8 -*-
"""
Created on Sun Apr 28 19:54:38 2019
...
def read(filename):
    df = pd.read_csv(filename, nrows=4670)
    print(df.head())
    label = df['group_100']
...
train_data,train_target=read('group_100_balanced.csv')
...
def gen2():
    ...
    while True: 
        for i in range(0,467):
            iteru = []
            iteru1 = []
            itera = islice(data_test, i*2, (i+1)*2)
            itera1 = islice(label_test, i*2, (i+1)*2)
            for img in itera:
                iteru.append(img)
            for img in itera1:
                iteru1.append(img)
...
tr_gen1 = gen1()
tr_gen2 = gen2()
m_check=keras.callbacks.ModelCheckpoint(filepath='./group_100_augment500.h5',monitor='val_fmeasure', verbose=0, save_best_only=True, mode='min')
...
file_path='./group_100_augment500.h5'
