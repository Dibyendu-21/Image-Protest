...
model = Sequential()
model.add(Conv2D(8, kernel_size=(3, 3), padding='valid', strides=(2, 2),input_shape=input_shape))
model.add(BatchNormalization(axis=-1))
model.add(Activation('relu'))
model.add(MaxPooling2D(pool_size=(4, 4)))
model.add(Conv2D(16, kernel_size=(4, 4), strides=(1, 1), padding='valid'))
model.add(BatchNormalization(axis=-1))
model.add(Activation('relu'))
...
m_check=keras.callbacks.ModelCheckpoint(filepath='./Leader_Bounding_Box_Custom_model_original_batch_before_relu.h5',monitor='val_loss', save_best_only=True, verbose=0)
...
def create_newSubmission(train_fname, train_predictions):
    ...
    sub_file = os.path.join('leader_python', 'leader_filename_train_originial_model_batch_before_relu' + '.csv')
    result_total.to_csv(sub_file, index=False)
    sub_file_1 = os.path.join('leader_python', 'leader_coordinate_train_originial_model_batch_before_relu' + '.csv')
...
def create_newSubmission_test(test_fname, test_predictions):
    ...
    if not os.path.isdir('leader_python'):
        os.mkdir('leader_python')
    sub_file = os.path.join('leader_python', 'leader_filename_test_originial_model_batch_before_relu' + '.csv')
    result_total.to_csv(sub_file, index=False)
    sub_file_1 = os.path.join('leader_python', 'leader_coordinate_test_originial_model_batch_before_relu' + '.csv')
    result_total1.to_csv(sub_file_1, index=False)
