...
m_check=keras.callbacks.ModelCheckpoint(filepath='./Leader_Bounding_Box_Custom_model_original.h5',monitor='val_loss', save_best_only=True, verbose=0)
...
def create_newSubmission(train_fname, train_predictions):
    ...
    sub_file = os.path.join('leader_python', 'leader_filename_train_originial_model' + '.csv')
    result_total.to_csv(sub_file, index=False)
    sub_file_1 = os.path.join('leader_python', 'leader_coordinate_train_originial_model' + '.csv')
...
def create_newSubmission_test(test_fname, test_predictions):
    ...
    if not os.path.isdir('leader_python'):
        os.mkdir('leader_python')
    sub_file = os.path.join('leader_python', 'leader_filename_test_originial_model' + '.csv')
    result_total.to_csv(sub_file, index=False)
    sub_file_1 = os.path.join('leader_python', 'leader_coordinate_test_originial_model' + '.csv')
    result_total1.to_csv(sub_file_1, index=False)
