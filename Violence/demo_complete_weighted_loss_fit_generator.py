...
def read(filename):
    ...
    df = pd.read_csv(filename,nrows=200,encoding='utf8')
...
train_data,train_target1,train_target2=read(r'C:\Users\Sonu\Documents\M.TECH\Research\UCLA-protest\img\new\Augment Images\Augment_train.csv')
...
def gen():
    ...
    while True: 
        for i in range(0,4):
            iteru = []
            itera = islice(data_train, i*8, (i+1)*8)
            itera1 = target1_train[ i*8: (i+1)*8]
            itera2 = target2_train[ i*8: (i+1)*8]
            for img in itera:
                iteru.append(img)
...
def gen1():
    ...
    while True: 
        for i in range(0,1):
            iteru = []
            itera = islice(data_holdout, i*8, (i+1)*8)
            itera1 = target1_holdout[ i*8: (i+1)*8]
            itera2 = target2_holdout[ i*8: (i+1)*8]
            for img in itera:
                iteru.append(img)
            yield np.array(iteru), [np.array(itera1), np.array(itera2)]
...
test_batch_length = len(data_test)
...
model1.compile(loss=[weighted_binary_crossentropy1, weighted_binary_crossentropy2], optimizer='adam',metrics=[fmeasure])
tr_gen = gen()
tr_gen1 = gen1()
m_check=keras.callbacks.ModelCheckpoint(filepath='./violence_visual.h5',monitor='loss', verbose=0, save_best_only=True, mode='min')
reduce_lr = keras.callbacks.ReduceLROnPlateau(monitor = 'val_loss' , factor=0.2 , patience=5 , verbose=1 , min_lr=0.0001)
history=model1.fit_generator(generator=tr_gen, steps_per_epoch=count, nb_epoch=5, validation_data=tr_gen1, validation_steps=count1, max_queue_size=2,callbacks=[m_check, reduce_lr])
score = model1.evaluate(np.array(data_test),[np.array(target1_test),np.array(target2_test)], verbose=0)
print('Score: ', score)
score = model1.evaluate(np.array(data_holdout),[np.array(target1_holdout),np.array(target2_holdout)], verbose=0)
