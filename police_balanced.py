# -*- coding: utf-8 -*-
"""
Created on Sun Apr 28 20:21:56 2019
...
def read(filename):
    df = pd.read_csv(filename, nrows =5100)
    print(df.head())
    label = df['police']
...
train_data,train_target=read('police_balanced.csv')
...
def gen():
    ...
    while True: 
        for i in range(0,204):
            iteru = []
            iteru1 = []
            itera = islice(data_train, i*16, (i+1)*16)
            itera1 = islice(label_train, i*16, (i+1)*16)
            for img in itera:
                iteru.append(img)
            for img in itera1:
                iteru1.append(img)
...
def gen2():
    ...
    while True: 
        for i in range(0,102):
            iteru = []
            iteru1 = []
            itera = islice(data_test, i*10, (i+1)*10)
            itera1 = islice(label_test, i*10, (i+1)*10)
            for img in itera:
                iteru.append(img)
            for img in itera1:
                iteru1.append(img)
...
tr_gen1 = gen1()
tr_gen2 = gen2()
m_check=keras.callbacks.ModelCheckpoint(filepath='./police_augment500.h5',monitor='val_fmeasure', verbose=0, save_best_only=True, mode='min')
...
file_path='./police_augment500.h5'
